-- Hardening follow-up: isolate each mutation's working state and return a
-- deterministic synthetic tombstone if a client references a compacted record.
...
begin
  ...
  for mutation in select value from jsonb_array_elements(p_mutations) loop
    ...
    if mutation_id is null or char_length(mutation_id) not between 1 and 100 then
