    ...
    <div className="space-y-8">
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        ...
        <Panel title="Meal plan" empty="Plan a few meals locally; no nutrition claims or cloud calendar required.">
          <div className="grid gap-2 py-3 sm:grid-cols-[1fr_auto_auto_auto]"><input value={planTitle} onChange={(event) => setPlanTitle(event.target.value)} placeholder="Recipe or meal" className="min-h-10 rounded-card border border-cardamom bg-rice px-3" /><input type="date" value={planDate} onChange={(event) => setPlanDate(event.target.value)} aria-label="Meal plan date" className="min-h-10 rounded-card border border-cardamom bg-rice px-3" /><select value={planMeal} onChange={(event) => setPlanMeal(event.target.value as MealPlanEntry["meal"])} aria-label="Meal" className="min-h-10 rounded-card border border-cardamom bg-rice px-3"><option>breakfast</option><option>lunch</option><option>dinner</option><option>snack</option></select><button onClick={() => void addPlan()} className="rounded-card bg-turmeric px-3 font-semibold">Plan</button></div>
